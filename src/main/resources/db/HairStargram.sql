...
DROP TABLE deginerFavorite CASCADE CONSTRAINTS;
...
COMMENT ON COLUMN userRegist.businessNumber is '사업자번호';
...
DROP SEQUENCE company_comNo_SEQ;

CREATE SEQUENCE company_comNo_SEQ NOMAXVALUE NOCACHE NOORDER NOCYCLE;

COMMENT ON TABLE company is '사업자 등록 테이블';
COMMENT ON COLUMN company.comNo is '순번';
...
COMMENT ON COLUMN hairStyle.categoriNo is '카테고리';
...
/**********************************/
CREATE TABLE deginerFavorite(
		DFavoriteNo                   		NUMBER(10)		 NOT NULL,
		designerId                		VARCHAR2(100)		 NULL ,
		userId                        		VARCHAR2(100)		 NULL 
);

DROP SEQUENCE deginerFavorite_DFavoriteNo_SEQ;

CREATE SEQUENCE deginerFavorite_DFavoriteNo_SEQ NOMAXVALUE NOCACHE NOORDER NOCYCLE;

COMMENT ON TABLE deginerFavorite is '디자이너 즐겨찾기 테이블';
COMMENT ON COLUMN deginerFavorite.DFavoriteNo is '디자이너 즐겨찾기 번호';
COMMENT ON COLUMN deginerFavorite.designerId is '디자이너 아이디';
COMMENT ON COLUMN deginerFavorite.userId is '사용자 아이디';
...
ALTER TABLE company ADD CONSTRAINT IDX_company_FK0 FOREIGN KEY (userId) REFERENCES userRegist (userId);
...
ALTER TABLE reservation ADD CONSTRAINT IDX_reservation_FK0 FOREIGN KEY (userId) REFERENCES userRegist (userId);
ALTER TABLE reservation ADD CONSTRAINT IDX_reservation_FK1 FOREIGN KEY (styleNo) REFERENCES hairStyle (styleNo);
ALTER TABLE reservation ADD CONSTRAINT IDX_reservation_FK2 FOREIGN KEY (designerId) REFERENCES userRegist (userId);

ALTER TABLE deginerFavorite ADD CONSTRAINT IDX_deginerFavorite_FK0 FOREIGN KEY (userId) REFERENCES userRegist (userId);
ALTER TABLE deginerFavorite ADD CONSTRAINT IDX_deginerFavorite_FK1 FOREIGN KEY (designerId) REFERENCES userRegist (userId);
...
ALTER TABLE useReply ADD CONSTRAINT IDX_useReply_FK0 FOREIGN KEY (userId) REFERENCES userRegist (userId);
ALTER TABLE useReply ADD CONSTRAINT IDX_useReply_FK1 FOREIGN KEY (designerId) REFERENCES userRegist (userId);
